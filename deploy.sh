...
migrate_database() {
    ...
    if command -v npm &> /dev/null; then
        ...
        docker compose -f docker/prod/docker-compose.yml run --rm backend npm run db:migrate:prod
    fi
...
check_health() {
    ...
    # 检查数据库
    if docker compose -f docker/prod/docker-compose.yml exec -T db pg_isready -U postgres > /dev/null 2>&1; then
        log_success "数据库服务正常"
    else
        log_error "数据库服务异常"
    fi
...
view_logs() {
    ...
    if [[ -n "$service" ]]; then
        ...
        docker compose -f docker/prod/docker-compose.yml logs -f "$service"
    else
        log_info "查看所有服务日志..."
...
backup_database() {
    ...
    docker compose -f docker/prod/docker-compose.yml exec -T db pg_dump -U postgres newspaper > "$backup_file"
